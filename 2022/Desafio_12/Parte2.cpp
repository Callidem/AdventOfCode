...
int main(){
    ...
    while(!inst.empty()){
        if(inst.front() == "noop") {
            ...
            if(cycle % 40 == 0) image << endl;
            if(reg == cycle % 40 || reg+1 == cycle % 40 || reg+2 == cycle % 40){
                cout << "Reg:" << reg << endl;
                image << '#';
            }
            else image << '.';
    ...
    cout << "Resposta: " << endl << image.str();
    stringstream hdImage, sString;
    while(!image.eof()){
        string line;
        getline(image, line);
        sString << line;
        ...
        getline(hdImage, line);
        for(int i = 0; i < 3; i++){
            hdImage << line << endl;
        }

    }
    cout << "Resposta: " << endl << hdImage.str();
}