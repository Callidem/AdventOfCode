...
int main (void){
    ...
    for(int i = 0; i < cards.size(); i++){
        sum += cards[i].getCopies();
        copySeq = cards[i].getPoints();
        factor = cards[i].getCopies();
        //cout << "Card " << i << ": " << copySeq << " Points | " << factor << " Copies" << endl;
        //cout << "total cards: " << cards.size() << "| i: " << i << endl;
        for(int j = 0; j < factor; j++){    
            for(int k = i+1; k <= i+copySeq && k < cards.size(); k++){
                //cout << "k: " << k << endl;
                cards[k].addCopy();
            }
        }
